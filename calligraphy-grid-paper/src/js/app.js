...
function applyConfig() {
  ...
  if (config.paperTexture === 'light') {
    elements.paper.classList.add('texture-light');
  } else if (config.paperTexture === 'heavy') {
    elements.paper.classList.add('texture-heavy');
  }
}
...
function createGridCell(index) {
  ...
  if (row > 0) {
    borderTop = config.innerLineWidth;
  }
  if (col < config.cols - 1) {
    borderRight = config.innerLineWidth;
  }
  if (row < config.rows - 1) {
    borderBottom = config.innerLineWidth;
  }
  if (col > 0) {
    borderLeft = config.innerLineWidth;
  }
